...
> **Version**: 0.3.1（Vercel AI SDK 架構 + D1–D6 定案 + 兩輪審視修正）
> **Created**: 2026-07-09 ｜ **Updated**: 2026-07-09
> **對準 SDK 版本**: Vercel AI SDK **v6**（`maxOutputTokens` / `inputTokens` 系列 / `FilePart` / `output:'no-schema'` 皆 v5 引入、v6 沿用）
...
- **請求組裝快照**：對每個呼叫點快照 gateway 產生的 AI SDK 參數，確保遷移不改變 gateway 層送出內容（**非**要求 LLM 回應逐位元一致）。⚠️ 此快照**保證不了** AI SDK→provider 的 wire 與現行手寫版相同（見 §3.8），故 Azure 路徑（含 extraction 三階段）也須配合**行為/準確率驗證**。
