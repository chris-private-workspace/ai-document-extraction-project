...
> **Status**: 🟢 D1–D7 定案；規格待用戶決定 spike（D8）/scope（D10）後排實作（**尚未寫入 `sprint-status.yaml`**）
> **Epic Key**: EPIC-23（暫定）
> **前置**: CHANGE-099（已完成）
...
> 尚待用戶拍板：**D8**（是否先 spike）、**D9**（confidence per-model 校準做法，見 §6.1）、**D10**（scope 縮減）、**D11**（營運骨架歸屬）—— 詳見 `senior-review-v0.3.1.md` §7。
...
> ⚠️ 三輪審視建議先做 **Phase 0 spike**（1-2 天，真實文件 × 2-3 provider 量準確率 + confidence 分佈）決定投資規模 —— 待 **D8** 用戶拍板。以下 Story 已納入三輪審視補的硬缺口。
...
- **建立/更新**：2026-07-09
- **版本**：0.4.0
- **狀態**：🟢 D1–D7 定案；規格待 D8/D10 決定；**未寫入 `sprint-status.yaml`**
