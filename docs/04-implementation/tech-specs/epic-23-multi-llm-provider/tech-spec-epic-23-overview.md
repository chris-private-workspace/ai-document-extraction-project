...
| **Rollout 安全** | 核心管線改動無 feature flag / shadow mode / rollback | **複用既有百分比灰度**（`FEATURE_EXTRACTION_V3_PERCENTAGE` + fileId 雜湊）；gateway 走 flag；shadow mode 新舊並行比對 confidence 分佈後再灰度切換｜**step 4 實作定案**（用戶批）：改用**專屬** `FEATURE_LLM_GATEWAY_ENABLED`（全域 on/off，語意不與 V3-vs-V2 混淆），切入點在 `gpt-caller.service.call()` 單一 chokepoint（簽章不變、覆蓋 Stage 1-3）；modelId 解析不到即回退舊 fetch。先**硬切換**，shadow mode + 百分比灰度（需把 fileId 串到 gpt-caller）留 **step 4b**。⚠️ image `detail` 轉發、AI SDK→wire 等價、per-model confidence 校準（§6.1，僅換非 Azure 時觸發）皆屬 4b 驗證項 |
