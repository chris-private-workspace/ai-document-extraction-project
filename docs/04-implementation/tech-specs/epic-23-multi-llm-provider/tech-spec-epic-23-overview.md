...
| 項目 | 現況缺口 | 要做 |
|------|----------|------|
| **用量持久化** | 🔴 `aiCostService.logUsage()` **零呼叫端**；主管線 token 只進 JSON blob → 成本儀表板只反映 term-validation | gateway 每次呼叫寫 `ApiUsageLog`（帶 `provider`/`model`/tokens/latency/success）；擴 `ApiProviderType` enum｜**step 5 完成**：gateway `recordObservability()` 於 `usageContext.documentId` 提供時反查 `document.cityCode` → `aiCostService.logUsage`（fire-and-forget，失敗不影響呼叫）；Stage3 已串（唯一有 documentId 者），Stage1/2 無 documentId 只記 log。⚠️ `ApiProvider` enum **已含 `AZURE_OPENAI`/`OPENAI`**，step 5 免擴；擴 enum 留非 Azure 的 **Story 23.3** |
| **結構化 logging** | AI 呼叫用 `console.*`，`logger.service` 沒接 | gateway 統一走 `logger.service`，標 provider/model/latency/tokens｜**step 5 完成**：每次呼叫走 `aiLogger`（`LogSource.AI`）記 provider/model/deployment/tokens/latency（成功 `info`、失敗 `warn`），fire-and-forget |
...
| **Rollout 安全** | 核心管線改動無 feature flag / shadow mode / rollback | **複用既有百分比灰度**（`FEATURE_EXTRACTION_V3_PERCENTAGE` + fileId 雜湊）；gateway 走 flag；shadow mode 新舊並行比對 confidence 分佈後再灰度切換｜**step 4 實作定案**（用戶批）：改用**專屬** `FEATURE_LLM_GATEWAY_ENABLED`（全域 on/off，語意不與 V3-vs-V2 混淆），切入點在 `gpt-caller.service.call()` 單一 chokepoint（簽章不變、覆蓋 Stage 1-3）；modelId 解析不到即回退舊 fetch。｜**step 4b 完成**（2026-07-10，用戶批選項 B）：① **image `detail` 轉發**——gateway `toFilePart` 經 `providerOptions.openai.imageDetail` 轉發（查證 `@ai-sdk/azure` 沿用 `OpenAIChatLanguageModel` 讀此 key，非臆測），補上 Stage 3 送圖時 wire 唯一缺口；② **一致性百分比灰度**——新增 `shouldUseLlmGateway(fileId)`（`FEATURE_LLM_GATEWAY_PERCENTAGE`，預設 100 維持硬切換語意；`simpleHash(fileId)%100<pct`），專屬 `fileId` 串三階段（orchestrator→stage1/2/3→gpt-caller，與計費用 `usageContext.documentId` 分離）；③ **`LlmCallPlan.assembledMessages` 請求組裝快照**（gateway 層 wire 等價半驗證）；④ **offline 影子 harness** `scripts/epic-23-spike/stage3-shadow-comparison.ts`（新舊路徑同批重放比對 confidence 分佈/欄位一致率/路由 delta）。⚠️ **仍待**：AI SDK→wire **實跑**等價（跑上述 harness，需真實 Azure 憑證）；per-model confidence 校準（§6.1，僅換非 Azure 時觸發，歸 **Story 23.3**，本批全 Azure 不觸發） |
