# Story 23.3 — 信心度路由 per-model 校準：設計文件（P1 已交付；P2 已決議不執行）
...
| **P2** ❌ **已決議不執行（2026-07-27，OQ-E）** | ~~校準流程腳本 + gold set 實測 + 逐一填 per-model 閾值 + §6.2 準確率回歸整合~~ | ~~gold set~~ | — |
| **P3（中長期）** | 監控哨兵 + 降低 confidence 自評依賴（改倚重 FIELD_COMPLETENESS / 跨欄位驗證 / 金額對帳等確定性訊號） | ~~P1/P2~~ P1 | 路由演算法演進（H1） |

> ⚠️ **P2 已於 2026-07-27 決議不執行**（見 §7.0）：核心提取不換 Azure，非 Azure 定位為可選備援 → 校準路線無業務需求驅動，且其前提已被實測推翻。
> P3 的「降低自評依賴」方向**與 P2 無依賴關係**，且已移交專案層的 [OQ-Q5](../../../open-questions.md) 追蹤 —— 它處理的是現行 Azure 流程的既有問題，不需要等任何 Epic 23 的東西。
...
| # | 問題 | 現況 |
|---|------|------|
| OQ-A | gold set 來源與規模 | ✅ **已解除（2026-07-27）**：它只對選項 A 是強制前提，而 OQ-E 決議不走該路線（見 §7.0）→ 不再阻塞任何交付。若日後為 [OQ-Q5](../../../open-questions.md) 的方向 3 而需要量測基準，再重啟本議題（屆時先查 Azure DEV 的審核記錄，勿直接投入人工標註——本地實測**無法建立**：僅 9 份可用、2 間公司、全 AUTO_APPROVE） |
...
| **OQ-E** | **P2 的前提已被實測推翻。根因（自評不可靠）已拆出為 [OQ-Q5](../../../open-questions.md)；本 OQ 收斂為「Epic 23 側該如何處置」** | ✅ **已決議（2026-07-27）—— 降級，P2 不執行**。使用者確認核心提取**不**打算換掉 Azure，非 Azure 定位為「可選的備援選項」→ 命中 §7.1〈決策所需輸入〉第 1 點。詳見下方 §7.0 |

### 7.0 ✅ OQ-E 決議（2026-07-27）

> **決策者**：使用者
> **決議**：**降級 —— 整條 per-model 校準路線（P2）不執行**

#### 使用者的決策原文意旨

> 核心提取不是真要換掉 Azure，只是準備一個可選的選項。

這正是 §7.1〈決策所需輸入〉第 1 點所描述的情況：商業動機是**備援／避免鎖定**（D7 早已如此定義），而非「Stage 3 必須改用非 Azure」。

#### 決議內容

| 項目 | 決議 |
|---|---|
| **P2（校準流程腳本 + gold set 實測 + 逐一填 per-model 閾值）** | ❌ **不執行**。前提已被實測推翻，且此決議下也無業務需求驅動它 |
| **P1（per-model 閾值地基）** | ✅ **保留**。維持「可調旋鈕」定位——未來若真要換、或 OQ-Q5 的處置需要 per-model 差異化，機制已在，不必重做。未校準時全部落全域 90/70，**行為零變** |
| **OQ-A（gold set 來源與規模）** | ✅ **解除**。它只對選項 A 是強制前提，本決議下不再阻塞任何交付 |
| **`getStageModel` 的 Azure 閘門**（`llm-model-config.service.ts:205`） | ✅ **保留不動**。它原本是 D9 未定案的暫時閘門，現在成為此決議的**執行機制**——非 Azure provider 可設定、可測試、可用於不影響路由的環節，但不會被指派到核心提取 |
| **非 Azure 的實際用途** | 備援選項：provider 設定、憑證、連線測試、熔斷、failover 皆已具備；真要啟用時是設定問題，不是開發問題 |
| **[OQ-Q5](../../../open-questions.md)（自評不可靠）** | **不受本決議影響**，維持 🟡 Open。它是現行 Azure 流程的既有問題，與換不換 provider 無關 |

#### 這個決議消除了什麼

原本的推論鏈是「要換 provider → 需校準 → 需 gold set → 本地建不出來 → 卡住」。決議把第一環拿掉之後，後面三環一併失效。Epic 23 的交付範圍回到它原本該有的樣子：**建立切換能力**，而不是**執行切換**。

#### 這個決議**沒有**消除什麼

- 非 Azure 若哪天真要用於核心提取，安全閘的問題會原封不動回來（屆時重讀 §7.1 的選項 A/B/C，建議仍是 C）。
- OQ-Q5 依然存在，且影響的是每天實際在跑的量。**不要因為 Epic 23 這條線收掉了，就把 OQ-Q5 一起忘掉。**
...
> **決策者**：使用者
> **狀態**：✅ 已於同日決議（見上方 §7.0）—— 以下保留完整分析，作為未來若重啟此議題時的依據
...
- **v0.5**：2026-07-27 — ✅ **OQ-E 已決議（§7.0）**：使用者確認核心提取**不**換 Azure、非 Azure 定位為可選備援 → **P2 不執行**、OQ-A 解除、P1 保留為可調旋鈕、`getStageModel` 的 Azure 閘門由「暫時措施」轉為「此決議的執行機制」。§7.1 的完整分析保留作為未來重啟時的依據。
- **下一步**：本文件的 Epic 23 部分**已收束**——P1 已交付、P2 不執行、OQ-A/B/C/E 皆已了結，僅 OQ-D（監控哨兵複用 `ApiUsageLog` 與否）待 P3 定。所有模型落全域 90/70，行為零變；非 Azure 不用於核心提取（`getStageModel` 的 Azure 閘門保留）。
  🔴 **唯一仍在流動的是 [OQ-Q5](../../../open-questions.md)**——它不屬於本文件，也不因本決議而消失。
