...
> **最後更新**：2026-07-25（FIX-133 盤點：新增 #12 `unique_template_mapping` 唯一約束從未生效） | **來源**：`docs/06-codebase-analyze/` 驗證報告 R1-R15（44 份）
...
| 12 | **`unique_template_mapping` 唯一約束從未生效（NULL 語意）+ Tech Spec 自相矛盾** | `TemplateFieldMapping` 的 `@@unique([dataTemplateId, scope, companyId, documentFormatId])`（`schema.prisma:3104`）因 PostgreSQL 預設 `NULLS DISTINCT` 而**對任何範圍都不生效** —— 每種範圍都必然含至少一個 NULL 欄（GLOBAL 兩個、COMPANY/FORMAT 各一個），故相同四元組可無聲重複建立（2026-07-25 實測回 201）。此知識早在 `prisma/seed-template-field-mappings.js:277` 註解載明（CHANGE-101，2026-07-09），但當時只當該腳本的局部 workaround，未升級為 schema 議題。另 `tech-spec-story-19-1.md:131` 的 `// 同範圍內的優先級` 與 `:144` 的「確保同一模版+範圍+公司/格式的組合唯一」**自相矛盾** | **FIX-133**：新重複已由 **CHANGE-107** 的應用層檢查擋下（`service.create()` 四元組 + `isActive: true` → 409，PR #144）。2026-07-25 唯讀盤點：本地 0 組；**Azure DEV 2 組（4 筆），其中 CEVA Inbound 一組兩筆同時啟用 → 6 個 `targetField` 重疊、`docs_fee`/`handling` 取值來源互相衝突，勝負取決於 PG 實體列順序（非確定性）**。BUG-3 判定為**資料污染非刻意分層**（全表 36+4 筆 `priority` 皆為 0，從未使用）→ 清理（B-1）與 `NULLS NOT DISTINCT` migration（C）**待使用者決策**，B-1 另需業務判斷正確來源欄位 |
