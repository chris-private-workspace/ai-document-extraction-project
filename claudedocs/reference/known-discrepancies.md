...
| 13 | **CEVA 兩個新增欄位定義的 aliases 未經真實發票驗證**（FIX-158 技術債務） | `field_definition_sets` 的 `f13aaf3b-ec74-4750-8036-a27dbb554792`（`CEVA LOGISTICS (HONG KONG) LTD`，companyId `0d02b680`）於 2026-08-03 補入 4 個欄位定義。其中 `destination_gate_fee` 與 `destination_truck_servicing_fee` 的 aliases 是**依「X at Destination」書寫模式推導**，非觀察真實發票得來 —— 全庫 88 份 CEVA 提取結果、33 種行項描述中這兩類費用**零實例**。另兩者尚有旁證（`emergency_fuel_surcharge` ← DHL `FUEL SURCHARGE` ×34；`destination_cfs_charges` ← `(SEA) CFS (DEST)`、`CFS CHARGES`）。aliases 會進 Stage 3 prompt，用字不對則模型抽不到 | 待一張含該費用的真實 CEVA 發票驗證。**若抽不到 → 以發票原文回填 aliases，不可移除欄位**（§樣本 ≠ 母體）。不動 `CEVA LOGISTICS (HONG KONG) LIMITED`（7448b7c5）—— 該公司已有 `cfs` / `gate_charge`，補了會製造語意雙胞胎，即 FIX-158 問題一的成因 |
