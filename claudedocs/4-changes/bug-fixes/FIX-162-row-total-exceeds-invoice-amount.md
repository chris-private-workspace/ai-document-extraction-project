...
上表「其他 10 份」中有兩組異常，**不屬本 FIX 已確認的兩類**，列此待查：

| 文件 | 欄位合計 | 基準 | 差 | 疑似 |
|---|---:|---:|---:|---|
| `DHL_RCEX240640_53822` | 24,943.22 | 5,832.47 | **+19,110.75** | 量級不對，形態近似 [FIX-152](FIX-152-dhl-multi-shipment-aggregate-amount-leak.md) 的多筆併單合計外洩 |
| `DHL_RCEX240709_99161`、`DHL_RCEX250347_61464`、`DHL_RCEX250410,0411,0412_69413` | — | — | 各 **−40.00** | 三份差額完全相同，指向某個固定費用無欄位承接（[FIX-160](FIX-160-template-mapping-unreferenced-extracted-charges.md) 方向） |

三份同為 −40 這點值得注意 —— 固定金額的重複出現通常指向規則層而非個別文件，與 §一個可能的規律 對 `RIL_RHIM260091` / `260092` 超出額相同的推論同型。
...
**建立者**: AI 助手
**最後更新**: 2026-08-08（B 類獨立複驗：形態與比例獲支持，補入未列入的 `DHL_RCEX240682_57590`；記錄兩組待查訊號）
