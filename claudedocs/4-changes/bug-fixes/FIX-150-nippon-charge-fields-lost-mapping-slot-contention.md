...
> **狀態**: 🚧 進行中（防護腳本、NEHK B/L fee、VAT 獨立成欄三項**設定變更皆已寫入本機與 Azure DEV 並回讀確認**；**映射層效果已於 2026-08-04 驗證** —— Azure DEV 對 59 份 NEX 文件跑 `preview`，`vat` **59/59 有值**，見 §映射層驗證。**提取層已查明**：那 16 個來源大多只出現在**進口**發票，而本 FIX 改的是 Outbound 映射 —— 50 份出口樣本中有 12 個來源一次都沒出現，故原驗收標準無法達成，見 §那 16 個來源大多只出現在進口發票。Outbound seal fee 待使用者決定）
...
### 🔴 那 16 個來源大多只出現在**進口**發票（2026-08-04，本機 98 份樣本）

以使用者提供的 375 份樣本在本機跑第二批（197 份，Nippon 系 **98 份**有提取結果：進口 48 / 出口 50），逐一統計每個來源欄位的有值份數：

| 分類 | 來源欄位 | 進口 | 出口 |
|---|---|---:|---:|
| **只在進口（12 個）** | `nvo_freight`、`nehk_do_fee` | 20/48 | **0/50** |
| | `low_sulphur_surchg` | 18/48 | **0/50** |
| | `ocean_freight`、`other_charges`、`do_fee` | 13/48 | **0/50** |
| | `cleaning_container` | 9/48 | **0/50** |
| | `status_charge`、`facility_charge` | 5/48 | **0/50** |
| | `port_security_charge`、`o_local_truckage`、`empty_container_placement` | 1/48 | **0/50** |
| **出口也有（3 個）** | `cfs_charge` | 7/48 | 3/50 |
| | `o_gate_io_or_parking_chg` | 1/48 | 1/50 |
| | `surrender_bl` | **0/48** | **5/50** ← 只在出口 |
| **兩邊皆無（1 個）** | `t_h_c` | 0/48 | 0/50 |

50 份出口發票的行項描述僅 18 種，四種即佔絕大多數：

```
55× THC        49× HANDLING CHARGES        46× B/L FEE        43× SEAL CHARGE
```

其餘 14 種皆為個位數，含 `SURRENDER B/L` ×5、`CFS CHARGES` ×3、`PARKING CHARGES` ×2 —— 正對應上表「出口也有」的那三個來源。

**這解釋了本 FIX 的驗收為何始終驗不到。** §預期數值變化 與 §驗收現況盤點 挑的都是 Outbound（出口）實例，而變更 A/C 在 Outbound 映射新增的規則，其對應費用在出口發票上大多不存在。

⚠️ **不要據此推論「規則加錯層級」** —— 資料不支持這個說法：

- `surrender_bl` **只在出口**出現（5/50，進口 0/48），Outbound 的 `document_fee` 公式引用它是正確的
- `cfs_charge`（3/50）與 `o_gate_io_or_parking_chg`（1/50）出口也有實例

準確的陳述是：**Outbound 映射的 14 條規則共引用 16 個來源欄位，其中 12 個在 50 份出口樣本裡一次都沒出現過**。這是母體分佈事實，不是規則錯誤 —— 但它意味著那 12 個來源所屬的規則，其實際效用尚無任何證據支持，且短期內也難以取得。

> 若要驗證變更 A/C 的完整效果，應改用**進口**實例（Inbound 模板），或接受「這些規則為罕見情境預留」的定位並在驗收標準中註明。

> 📌 統計基準：本表為 2026-08-04 補回 7 份 429 失敗文件（見下方註）**之後**的數字。補回前為 91 份（43/48），`o_gate_io_or_parking_chg` 出口曾為 0 —— 補回後出現 1 筆，「只在進口」的欄位數由 13 降為 12。**小樣本下的「0」極易翻轉，引用本表數字時務必連同分母一起引用。**

> ⚠️ 那 7 份文件初次處理時狀態為 `OCR_FAILED`，實際錯誤是 **GPT API 429 rate limit**（一次送 197 份、約 590 次呼叫打爆 `gpt-5.6-luna-aidocprocessing` 部署配額），與 OCR 無關。間隔 25 秒逐一重試後 7/7 成功。狀態命名會誤導診斷方向，另見待辦。
