...
> **狀態**: 🚧 進行中（防護腳本、NEHK B/L fee、VAT 獨立成欄三項**設定變更皆已寫入本機與 Azure DEV 並回讀確認**；**映射層效果已於 2026-08-04 驗證** —— Azure DEV 對 59 份 NEX 文件跑 `preview`，`vat` **59/59 有值**，見 §映射層驗證。**提取層已查明**：那 16 個來源幾乎只出現在**進口**發票，而本 FIX 改的是 Outbound 映射 —— 48 份出口樣本中 12 條規則的來源一次都沒出現，故原驗收標準無法達成，見 §那 16 個來源幾乎只出現在進口發票。Outbound seal fee 待使用者決定）
...
### 🔴 那 16 個來源幾乎只出現在**進口**發票（2026-08-04，本機 91 份樣本）

以使用者提供的 375 份樣本在本機跑第二批（197 份，Nippon 系 91 份有提取結果：進口 43 / 出口 48），逐一統計每個來源欄位的有值份數：

| 來源欄位 | 進口 | 出口 |
|---|---:|---:|
| `nvo_freight`、`nehk_do_fee` | 17/43 | **0/48** |
| `low_sulphur_surchg` | 15/43 | **0/48** |
| `ocean_freight`、`other_charges`、`do_fee` | 12/43 | **0/48** |
| `cleaning_container` | 8/43 | **0/48** |
| `status_charge`、`facility_charge` | 5/43 | **0/48** |
| `port_security_charge`、`o_local_truckage`、`o_gate_io_or_parking_chg`、`empty_container_placement` | 1/43 | **0/48** |
| `cfs_charge` | 7/43 | 2/48 |
| `surrender_bl` | **0/43** | **5/48** ← 只在出口 |
| `t_h_c` | 0/43 | 0/48 |

48 份出口發票的行項描述僅 18 種，四種即佔絕大多數：

```
53× THC        47× HANDLING CHARGES        45× B/L FEE        42× SEAL CHARGE
```

**這解釋了本 FIX 的驗收為何始終驗不到。** §預期數值變化 與 §驗收現況盤點 挑的都是 Outbound（出口）實例，而變更 A/C 在 Outbound 映射新增的規則，其對應費用在出口發票上幾乎不存在。

⚠️ **不要據此推論「規則加錯層級」** —— 資料不支持這個說法：

- `surrender_bl` **只在出口**出現（5/48，進口 0/43），Outbound 的 `document_fee` 公式引用它是正確的
- `cfs_charge` 出口也有 2 筆

準確的陳述是：**Outbound 映射的 14 條規則中，有 12 條所引用的來源費用在 48 份出口樣本裡一次都沒出現過**。這是母體分佈事實，不是規則錯誤 —— 但它意味著那 12 條規則的實際效用尚無任何證據支持，且短期內也難以取得。
