...
> **建立日期**: 2026-07-10
> **最後更新**: 2026-07-14（v3.4 —— 治本 + 緩解 + 收斂 1 已部署至 Azure DEV，映像 `dev-fix108-20260714135401`）
...
> **狀態**: ✅ **根因已確認（應用端事件迴圈飽和，非 DB 故障）；治本（§5.5）+ 臨時緩解（§5.1）+ 收斂 1（§5.3）已實作並於 2026-07-14 部署至 Azure DEV；收斂 2／止血／`PROCESS_CONCURRENCY` 實測調校尚未執行**
...
| 層 | 方向 | 具體內容 | 風險／備註 |
|----|------|----------|------------|
| **治本** | ✅ **已實作**（§5.5） | `upload/route.ts` 的 `Promise.allSettled(全部)` 改為 `runInBatches`（每批 `PROCESS_CONCURRENCY = 3`，批內併發、批間序列）。**無需新依賴** | 綁住同時載入記憶體的 PDF 數；批量大小待實測調校 |
...
### 5.5 已實施：治本 —— 分批限制併發處理數（2026-07-13）

| 項目 | 內容 |
|------|------|
| 改動 | `src/lib/upload/constants.ts`（新增 `PROCESS_CONCURRENCY: 3`）+ `src/app/api/documents/upload/route.ts`（新增 `runInBatches` helper，unified + legacy 兩分支改用） |
| Commit | `bc6543f` |
| 問題 | 上傳後 `Promise.allSettled(documentsToProcess.map(...))` 將**該批全部**文件同時投入處理，20 份 PDF 同時 buffer／圖片轉換／base64 → 記憶體 95%、事件迴圈凍結（§4.1） |
| 做法 | `runInBatches<T>(items, concurrency, label, worker)`：將 items 切成大小為 `concurrency` 的批次，**批內併發、批間序列**（`await Promise.allSettled(chunk)` 當屏障）。同時載入記憶體的 PDF 數因此被 `PROCESS_CONCURRENCY` 綁死，與單次上傳份數脫鉤 |
| 為何是治本 | §5.1（`MAX_FILES_PER_BATCH` 15）只降**上傳份數**、處理仍全併發，未移除觸發機制；本項才真正綁住峰值 → 上傳 15 或 50 份都安全 |
| 共用 helper | 兩分支（unified `documentsToProcess` / legacy `uploaded`）共用同一 harness，避免收斂 1 那種「一處修一處漏」；rejected 記錄邏輯（收斂 1）內建於 helper |
| 保留行為 | fire-and-forget（呼叫端不 `await`，末尾掛 `.catch()` backstop）；HTTP 回應照舊立即返回 |
| 代價 | 批次總時間變長（20 份分 7 批、每批等最慢一份跑完 OCR+GPT）—— 以時間換記憶體穩定 |
| 驗證 | `npx eslint` 通過；`npm run type-check` 剩餘 4 個錯誤全在 `src/services/llm/`（本地缺 Epic 23 套件），與本改動無關 |
| 🔴 待實測 | `PROCESS_CONCURRENCY = 3` 為保守起點。實際記憶體峰值須待 Azure 上真跑一次 20 份上傳才知，屆時再調校（**已上線但此實測尚未執行**） |
| 部署 | ✅ **已部署**至 Azure DEV（2026-07-14，映像 `dev-fix108-20260714135401`）。App Service 未設 `PROCESS_CONCURRENCY`，故採程式碼預設值 3 |
...
- [x] ~~**治本**：`upload/route.ts` 實作併發限制（`runInBatches`，每批 `PROCESS_CONCURRENCY`）~~（已完成 2026-07-13，commit `bc6543f`，見 §5.5；**已於 2026-07-14 部署，現已生效**）
...
- [x] ~~**部署**：治本 + 收斂 1 + 緩解手動部署至 Azure~~（已完成 2026-07-14，映像 `dev-fix108-20260714135401`；部署記錄 `docs/07-deployment/02-azure-deployment/deployment-records/2026-07-14-dev-fix108.md`）
- [ ] **部署後實測**：真跑一次 20 份上傳、量測記憶體峰值以調校 `PROCESS_CONCURRENCY`（目前為預設 3，未實測）
- [ ] 查 App Service Plan `ASP-RAPOSCM-AIDocProcessing-DEV` 的 SKU 與記憶體上限（§5.6）
...
- §4.5 的耗時為真實時間戳相減得出。
- 本文件原為調查記錄。截至 v3.4，已實作 **§5.5 治本**（commit `bc6543f`）、**§5.1 臨時緩解**（`35f7298`）、**§5.3 收斂 1**（`05ee290`），且以上改動**已於 2026-07-14 部署**至 Azure DEV（映像 `dev-fix108-20260714135401`）；**收斂 2**、**止血 A/B**、**`PROCESS_CONCURRENCY` 實測調校**仍未執行。
