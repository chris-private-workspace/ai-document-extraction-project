...
> **建立日期**: 2026-07-10
> **最後更新**: 2026-07-13（v3.3 —— 治本已實作：分批限制併發處理數，見 §5.5）
...
> **狀態**: ✅ **根因已確認（應用端事件迴圈飽和，非 DB 故障）；治本（§5.5）+ 臨時緩解（§5.1）+ 收斂 1（§5.3）已實作；收斂 2／止血尚未實作**
...
| 🔴 待實測 | `PROCESS_CONCURRENCY = 3` 為保守起點。實際記憶體峰值須待 Azure 上部署後真跑一次 20 份上傳才知，屆時再調校 |
| 部署 | **尚未部署**至 Azure（本專案手動部署）。上線前此治本不生效 |
...
- [x] ~~**治本**：`upload/route.ts` 實作併發限制（`runInBatches`，每批 `PROCESS_CONCURRENCY`）~~（已完成 2026-07-13，commit `bc6543f`，見 §5.5；**尚未部署，上線前不生效**）
...
- [ ] **部署**：治本 + 收斂 1 + 緩解需一併手動部署至 Azure 才生效（本專案手動部署），部署後真跑一次 20 份上傳、量測記憶體峰值以調校 `PROCESS_CONCURRENCY`
...
- §4.5 的耗時為真實時間戳相減得出。
- 本文件原為調查記錄。截至 v3.3，已實作 **§5.5 治本**（commit `bc6543f`）、**§5.1 臨時緩解**（`35f7298`）、**§5.3 收斂 1**（`05ee290`）；**收斂 2**、**止血 A/B** 仍未執行，且以上改動**尚未部署**至 Azure。
