...
> **狀態**: ✅ 已結案（2026-08-05 決定**維持現狀、不修改任何 mapping** —— 見 §結案決定與保留的已知影響）
...
**2026-08-05 決定：維持現狀，不修改任何 mapping。** 本 FIX 據此結案。
...
決定為**不動**。`template_field_mappings` 的 8 條規則全部保持原狀，Nippon 繼續獨立成欄、其餘四家繼續併入 handling。

### 🔴 因此以下三項成為**刻意保留**的已知狀態，不是待修缺陷

再次遇到時請勿當成新問題重開單：

| # | 保留的狀態 | 實務影響 |
|---|---|---|
| 1 | Nippon 的稅獨立成欄、其餘四家併入 handling，**兩種口徑並存** | 跨公司比較 handling 成本**不可直接比較** —— Nippon 的數字不含稅，其餘四家含稅。需要橫向比較時必須先在報表端還原 |
| 2 | 13 列的 `handling` / `handling_charge` **值 100% 來自稅額**（合計 193.43，全為 Toll） | 報表顯示「手續費 22.31」而實際手續費為 0。閱讀該欄時不可逕自視為服務費 |
| 3 | Inbound/Outbound Full List 模板的 `vat` 欄對四家**永遠空白** | 該欄不可作為「這批資料有沒有稅」的判準 |

### 判定為預期行為的依據

每條規則都按設定正確執行 —— 實測 844 列中，formula 應得值與模板實際值的不符次數為 **0**，稅額去向不明為 **0**。這不是程式缺陷，是配置選擇；既然選擇維持，即無工作項。

### 若日後要改變此決定

需重新評估的是 Nippon 的 2 條 `vat_7 → vat` DIRECT 規則（若統一為併入 handling），或其餘四家的 6 條 FORMULA（若統一為獨立成欄）。變更範圍與驗證方式見下方兩節，內容仍然適用。

⚠️ 規模參考：Nippon 的稅為 115 份合計 **80,814.79**，Toll 全部僅 1,050.99。把 Nippon 改為併入會使其 handling 大幅膨脹。
...
曾建議統一為方式 A（填進 `vat` 欄）：模板設計已預留該欄、Nippon 已在使用、「handling 100% 是稅」的顯示問題會自然消失。

**2026-08-05 決定不採納，維持現狀。** 上述三項影響因此持續存在，已列於 §結案決定。
...
**建立者**: AI 助手
**最後更新**: 2026-08-05（**結案**：決定維持現狀、不修改任何 mapping。三項已知影響轉為刻意保留，見 §結案決定）
