...
> **日期**: 2026-07-20
> **狀態**: ✅ 已實作（2026-07-20；本地 DB 已套用並驗證冪等；type-check / lint 通過）。⏳ 全域端到端驗證待做（見 §待驗證），Azure DEV 待套用
...
| 10 | Azure DEV 套用 | Kudu ad-hoc 執行 `update-stage2-prompt.js` | ⏳ |

## 待驗證

**GLOBAL prompt 尚未經端到端證明。** 目前 CEVA 的驗證是靠 COMPANY scope 繞法 `cmrso9em60002bsxghuih2rx0`，它的優先序高於 GLOBAL，所以即使 GLOBAL 修好了也不會被行使到。

驗證方式：把該 COMPANY 配置 `isActive` 設為 false → 重跑 CEVA 文件 B → 若仍命中格式 B，代表 GLOBAL prompt 生效，該繞法即可永久移除。
