...
> **狀態**: 🚧 部分完成 —— **本機已修復並雙向驗證通過**（gated 腳本，見 §驗證）；拆分後的設定缺口已於 **2026-08-06** 補齊（見 §拆分後的設定缺口）；⚠️ **Azure DEV 仍未套用**，且 2026-08-06 實測**確認該環境有同型誤歸、規模 35 份**，見 §跨環境狀態
> **最後更新**: 2026-08-06（Azure DEV 誤歸已由容器內唯讀診斷查證 —— 原記「尚未查證」不再成立；並發現該環境有兩種本機沒有的中英混排印法，移植時不能照抄本機 variants）
...
|---|---|
| 本機 | ✅ 已修復並驗證 |
| Azure DEV | ⚠️ **未套用**，且 **2026-08-06 已查證：該環境確實有同型誤歸，規模 35 份**（見下） |
