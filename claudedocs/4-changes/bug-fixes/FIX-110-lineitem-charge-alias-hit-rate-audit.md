...
### 9.7 一筆乾淨的未覆蓋實例：`RESIDENTIAL ADDRESS`（2026-08-08）

發現自 [FIX-162](FIX-162-row-total-exceeds-invoice-amount.md) §複驗 登記的待查訊號 —— 三份 DHL 文件的欄位合計與基準值差額**完全相同**，皆為 **−40.00**。追查後為同一筆費用。

| 檔案 | 行項 | 金額 | 名稱含 `residential` 的欄位 | 值等於 40 的欄位 |
|---|---|---:|---|---|
| `DHL_RCEX240709_99161.pdf` | `RESIDENTIAL ADDRESS` | 40 | 🔴 無 | 🔴 無 |
| `DHL_RCEX250138_96978.pdf` | `RESIDENTIAL ADDRESS` | 40 | 🔴 無 | 🔴 無 |
| `DHL_RCEX250347_61464.pdf` | `RESIDENTIAL ADDRESS` | 40 | 🔴 無 | 🔴 無 |
| `DHL_RCEX250410,0411,0412_69413.pdf` | `RESIDENTIAL ADDRESS` | 40 | 🔴 無 | 🔴 無 |

全庫 403 份可檢視文件中，含此行項者 **4 份，4/4 完全無承接**，合計 **160.00**。

這是本 FIX §9.2「A 類 —— 行項讀對了、費用沒有欄位承接」最乾淨的一個實例：

- `classifiedAs` **全部有值**（`Residential Address` / `Residential Address Charge`）—— GPT 分類正確
- description 乾淨、不含數字、無折行 —— 不屬 §3 根因 2（內嵌金額型），屬根因 3（乾淨但對不上 label）
- 金額固定 40.00（DHL 住宅地址附加費為固定費率），故三份差額完全相同
- 全 `claudedocs/` 查無任何 `residential` 記載 —— 此費用**從未被登記過**

⚠️ 金額規模小（160.00），**但它示範了 §9.2 的 35 筆「欄位數 < 行項數」是怎麼發生的**：不需要任何提取錯誤，只要欄位定義集裡沒有這個 key，費用就會安靜地消失。這類「固定金額在多份文件重複出現」的訊號，比單份的大額差異更容易定位根因。

**本節純量測，未新增任何欄位定義或 alias** —— 是否補 `residential_address` 欄位需依 §5 的判斷紀律另行評估（該費用僅見於 DHL，且只有 4 份）。

---

*文件建立日期: 2026-07-15*
*最後更新: 2026-08-08（新增 §9.7 `RESIDENTIAL ADDRESS` 未覆蓋實例；§9.2 的 8 筆遺留項追查完畢並加註閉環）*
*修訂歷史: 2026-08-07（新增 §9 金額影響量化：未覆蓋部分導致 49/372 份樣本欄位合計對不上，差額總和 68,991.52；§5 不全面補 alias 的結論維持不變）*
