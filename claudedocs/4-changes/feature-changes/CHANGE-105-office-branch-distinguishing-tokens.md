...
> **日期**: 2026-07-16
> **狀態**: ⏸️ 已取代（2026-07-16 同日 revert，前提被證據推翻；見文末「撤銷記錄」）
> **⚠️ 本 CHANGE 已撤銷**：合併(#114)後同日經實測與證據查核推翻前提，已 revert 回 CHANGE-103 行為（office/branch 仍為 generic）。保留本文件作審計記錄。
...
| 6 | 存量 Office 文件分流 | 重新處理後進 PENDING → 確認為新公司 | ❌ 未做（已撤銷） |
| 7 | Azure 部署 | 隨下次映像部署生效(純程式碼、無 schema/env) | ❌ 未部署（已撤銷） |

---

## 撤銷記錄（2026-07-16，同日 revert）

合併 #114 後，以實際重新處理 + DB 證據查核 CEVA 案例，推翻 CHANGE-105 的前提：

| 查核 | 證據 | 結論 |
|------|------|------|
| GPT 有無輸出「OFFICE」 | 重新處理 `CEVA_RCIM250004_05808`（POST /process 200、usedV3、CHANGE-105 在線）→ GPT `name = "CEVA LOGISTICS (HONG KONG) LTD"`、`rawText` 含 OFFICE 但錨定到已知 Ltd | **CHANGE-105 根本沒被觸發**：提取名已是 LTD、精確命中 canonical，token-set 的 office 路徑從未經過。CHANGE-105 在**問題下游**、無效 |
| Office 與 Ltd 是否同一實體 | 文件本身 `Cheques payable to "CEVA Logistics (Hong Kong) Limited"`、`HK - CEVA LOGISTICS (HONG KONG) LTD` | **同一法律/計費實體**；「Hong Kong Office」僅信頭辦公室標示。GPT 選 LTD 正確 |
| 兩份是否不同格式 | Stage 2 兩份都匹配**同一格式** `cmqur1q7...`（信心 86–93%） | 連格式都相同，非「多格式」問題 |

**推翻的前提**：CHANGE-105 假設「X Office = 不同計費實體、應分開」。但 CEVA 證據顯示 Office 通常只是同一 Ltd 的信頭標示；全域把「X Office」送 GRAY 審核只是**增加無謂人工負擔**，且 GPT 本就錨定到已知 Ltd（FIX-104）使本 CHANGE 對真實案例無效。

**撤銷內容**：`token-set.ts` 的 `GENERIC_COMPANY_TOKENS` 還原（office/branch 回為 generic）、兩測試檔還原為 CHANGE-103 期望。**若日後真需區分同集團營運實體，正確著力點在 Stage 1 known-company anchoring（FIX-104），非 token-set。**

> 使用者 2026-07-16 定案：公司維持同一間（不拆）；#114 已 merge 故以 revert 撤銷。
